-- Polygon migrated its exchange ticker from MATIC to POL. Binance no longer
-- serves MATIC/USDT, so keep default and existing watchlists on a valid pair.
...
SELECT user_id, 'POL/USDT', priority, TRUE
FROM watchlists
WHERE symbol = 'MATIC/USDT'
  AND is_active = TRUE
